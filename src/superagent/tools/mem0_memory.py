...
# NOTE: Modernized from the legacy TOOL_SPEC pattern to @tool (see the
# decorated function below). Schema is inferred from type hints + docstring.
...
def mem0_memory(
    ...
    """Memory management via Mem0 (store/get/list/retrieve/delete/history).

    Args:
        action: One of 'store', 'get', 'list', 'retrieve', 'delete', 'history'.
        content: Content to store (required for 'store').
        memory_id: Memory ID (required for 'get', 'delete', 'history').
        query: Search query (required for 'retrieve').
        user_id: User ID for the operation (required for 'store', 'list', 'retrieve').
        agent_id: Agent ID for the operation (alternative to user_id).
        metadata: Optional metadata to attach during 'store'.

    Returns:
        ToolResult-shaped dict with status and response content.
    """
    tool_input = {
        "action": action,
        "content": content,
